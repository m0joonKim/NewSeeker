...
from app.models import Alarm, FrequencyEnum, Message, DayOfWeekEnum
import uuid
...
def toggle_my_email_on(current_user: CurrentUser, session: SessionDep) -> Message:
    ...
    session.add(alarm)
    session.commit()
    return Message(message="Email notification toggled successfully")
...
def toggle_my_kakao_on(current_user: CurrentUser, session: SessionDep) -> Message:
    ...
    session.add(alarm)
    session.commit()
    return Message(message="Kakao notification toggled successfully")
...
def toggle_my_slack_on(current_user: CurrentUser, session: SessionDep) -> Message:
    ...
    session.add(alarm)
    session.commit()
    return Message(message="Slack notification toggled successfully")
