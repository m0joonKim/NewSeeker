...
def get_my_categories(current_user: CurrentUser, session: SessionDep) -> List[str]:
    ...
    my_categories = session.exec(statement.order_by(UserCategory.category_id)).all()
...
@router.get("/me/category_ids", response_model=List[int])
def get_my_category_ids(current_user: CurrentUser, session: SessionDep) -> List[int]:
    """
    Get preferred category IDs for the current user.
    """
    statement = select(UserCategory).where(UserCategory.user_id == current_user.id)
    my_categories = session.exec(statement.order_by(UserCategory.category_id)).all()
    
    # 카테고리 ID 조회
    category_ids = [uc.category_id for uc in my_categories]
...
def get_newspaper_categories(newspaper_id: int, session: SessionDep) -> List[str]:
    ...
    newspaper_categories = session.exec(statement.order_by(NewspaperCategory.category_id)).all()
